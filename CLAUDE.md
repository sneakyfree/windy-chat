...
**This repo:** LIVE — TWO emitters: agent-roster (`services/agent-roster` via `services/shared/admin-telemetry.js`) emits llm.call / roster.exchange / roster.tool_call / roster.quota_denied; onboarding emits hatch.agent_chat_provisioned / hatch.owner_chat_provisioned. Tokens `…__AGENT_ROSTER`, `…__CHAT_ONBOARDING`.

**Verify during your stress test** (read token `verify-oc5` in lockbox
`secrets/windy-admin/ingest-tokens.env`):
```bash
curl -s "https://admin.windyword.ai/v1/events/tail?limit=10&platform=windy-chat" \
...
Jobs stuck "Queued" = runner down, not billing: ssh Kit 0 → cd /home/github-runner/runners/windy-chat && sudo ./svc.sh status
